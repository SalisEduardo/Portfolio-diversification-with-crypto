...
In similar studies, Platanakis and Urquhart (2020), using weekly data collected from October 2011 to June 2018 ,concluded that Bitcoin provides higher risk-adjusted returns when added to portfolio composed of stocks (S&P500) and bonds (Barclays US Aggregare). Those results were consistent across different portfolio construction techniques, levels of risk aversion,time windows, transaction costs,constrains. The authors also stated that Bitcoin adds more value to the original portfolio than commodities investments.

Kajtazi and Moro (2018) considered a mean-CVaR (Conditional Value-at-Risk) approach for portfolio construction to evaluate the impacts of the inclusion of Bitcoin in a portfolio composed by  equities, fixed income, real state, cash equivalents, currencies, and alternative investments. Using data from USA, Europe and China, for the period from 2012 to 2017, the authors found that the inclusion of Bitcoin is able to provide better investment strategies in terms of risk and return, in terms of higher Sharpe, Omega and Sortino ratios. 
...
One of the most used methods to measure risk in the portfolio optimization process is the covariance matrix. Combined with the estimation of returns, it can cause a large impact on computing optimal weights by allocating a higher stake on securities with over estimated returns and underestimated risk or the opposite or by placing a minor part of the wealth on securities with under estimated expected returns and over estimated risks (Kolm, Tütüncü , Fabozzi, 2014). Those errors occur specially with standard estimator that is the sample covariance matrix. Thereby alternatives like shrinkage methods, factor models  or by time-varying estimators such as  Generalized Autoregressive Conditional Heteroskedasticity ("GARCH") or Exponentially Weighted Moving Average ("EWMA") models, are candidates to decrease the errors and thus enhance the performance.

Himbert, Kapraun and Rudolf (2017), focusing on the Minimum Variance Portfolio (that is not affected by returns estimation), found that EWMA and DDC-GARCH can improve covariance forecast and performance (specially for portfolios of higher dimension) in random built portfolio of stocks from US and Europe. Extending their analysis to Smart beta's portfolio and increasing the number of allocation strategies, the authors also identified superior results for those estimators, but only for the Minimum Variance strategy.
